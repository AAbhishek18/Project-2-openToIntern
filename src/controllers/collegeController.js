...
const createCollege = async function(req,res){
    ...
    if(Object.keys(college).length!=0){
       ...
       if(!college.fullName) return res.status(400).send({status:false,msg:'FisrtName is required'}) 
    ...
    const getCollege = async function(req,res){
        ...
        if(!requestCollege) return res.send({msg:'Plz, enter collegeName in query'})
        let collegeId = await collegeModel.findOne({name:requestCollege,isDeleted:false}).select({_id:1})
        if(collegeId==null) return res.send({msg:"College not exist"})
